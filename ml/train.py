...
def calculate_pandas_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Mock Sentiment & Macro (since we train on history, defaults)
    df['sentiment'] = 0.0
    df['fearGreed'] = 50.0
    
    # Lags (adds 25 features: lags 1 to 5 for return, rsi, macd_hist, volume_ratio, body_size)
...
def generate_rf_labels(df: pd.DataFrame, future_window=15, profit_target=0.015, stop_loss=0.0075) -> pd.Series:
    ...
    for i in range(len(df) - future_window):
        ...
        # Calculate percentage change for each future step
        ...
        for pct in pct_changes:
            if pct <= -stop_loss:
                # Stop loss hit first for buy
                sell_triggered = True
                break
            if pct >= profit_target:
                # Profit target hit first for buy
                buy_triggered = True
                break
                
        for pct in pct_changes:
            if pct >= stop_loss:
                # Stop loss hit first for sell
                buy_triggered = True
                break
            if pct <= -profit_target:
                # Profit target hit first for sell
                sell_triggered = True
                break
...
def train_pipeline(symbol: str):
    ...
    # Select feature columns (40+ features)
    ...
    # --- B. Isolation Forest (Anomaly Detector) ---
    logger.info("Training Isolation Forest Anomaly Detector...")
    # Fits on normal train data only
    iso = IsolationForest(contamination=0.05, random_state=42)
    iso.fit(X_train)
    
    # --- C. LSTM Predictor (Price Direction) ---
